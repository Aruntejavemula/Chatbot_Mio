...
if old_models in content_lf:
    content_lf = content_lf.replace(old_models, new_models)
    print('Replaced model list')
else:
    print('Model list not found')

# 2. Replace dropdown method using exact boundaries
old_dropdown_start = '  Widget _buildModelDropdown(bool isDark) {\n'
old_dropdown_end_marker = '\n  @override\n  Widget build(BuildContext context) {'

start_idx = content_lf.find(old_dropdown_start)
if start_idx != -1:
    end_idx = content_lf.find(old_dropdown_end_marker, start_idx)
    if end_idx != -1:
  ...
  @override
  Widget build(BuildContext context) {"""
        content_lf = content_lf[:start_idx] + new_dropdown + content_lf[end_idx:]
        print('Replaced dropdown method')
    else:
